...
class User {
   ...
   */
	...
	async saveFavoritedStories(story) {
		await axios({
		...
		console.log('saveFavoritedStories:', this.favorites);
	}
	...
	async removeFavoritedStories(story) {
		await axios({
			url: `${BASE_URL}/users/${this.username}/favorites/${story.storyId}`,
			method: 'DELETE',
			params: { token: `${this.loginToken}` }
		});
		this.removeFromFavorites(story);
		console.log('removeFavoritedStories:', this.favorites);
	}
}
